<template>
  <!-- fallbackOnMissing (home): 페이지가 없으면 fallback 슬롯을 단독 렌더 (자체 레이아웃 포함) -->
  <slot v-if="showFallback" name="fallback" />

  <div v-else class="theme-default">
    ...
    <main :class="['public-content-shell', `tpl-${template}`, { 'public-content-shell-flush': useBanner && showEyebrow }]">
      <template v-if="content">
        ...
        <article :class="['public-content', { 'with-hero': heroUrl && !useBanner, 'is-page': useBanner }]">
          ...
          <!-- Non-banner (basic): magazine-style meta card with eyebrow + title + byline. -->
      ...
      <section v-else class="public-content-error">
        <h1>404</h1>
...
type Content = {
  ...
  template?: string
  meta?: Record<string, unknown> & { showEyebrow?: boolean; template?: string; bannerTextColor?: string }
}
...
const props = withDefaults(defineProps<{
  contentType: string
  slug: string
  // When true (home): a missing page renders the #fallback slot instead of 404.
...
const template = computed(() => {
  const t = content.value?.template || content.value?.meta?.template
  if (t && VALID_TPL.includes(t)) return t
  return isPage.value ? 'narrow' : 'basic'
})
const useBanner = computed(() => template.value !== 'basic')
...
// home fallback: no content and not locked → render the #fallback slot alone
const showFallback = computed(() => props.fallbackOnMissing && !content.value && !locked.value)
...
useHead(() => {
  ...
  // home fallback / missing: don't override — let app.vue defaults (site name) stand.
  if (!c) return props.fallbackOnMissing ? {} : { title: '404' }
  return {
    title: c.title,
...
// Page mode ([slug]): a fetch error is a real 404. Home mode: swallow → fallback slot.
if (error.value && !props.fallbackOnMissing) {
...
/* Pages: top banner sits flush against the topbar. */
...
/* ── Hero ── */
.public-content-hero { position: relative; }
...
.public-content-eyebrow-link:hover { color: #1a6eb8; }
.public-content-eyebrow-sep { color: var(--theme-fg-faint); }
...
.public-content-tags a:hover { color: #1a6eb8; }
...
@media (max-width: 720px) {
  .public-content-shell { padding: 0 0 60px; }
  .public-content-hero img { aspect-ratio: 4 / 3; }
  .public-content-card { padding: 22px 20px 18px; }
  .public-content.with-hero .public-content-card {
    margin-top: -56px;
    margin-left: 16px;
    margin-right: 16px;
  ...
  .public-content-card h1 { font-size: 26px; }
  .public-content-summary { padding: 0 22px; font-size: 15px; }
  .public-content :deep(.block-renderer) { padding: 0 22px; }
  .public-content-tags { margin: 28px 22px 0; }
}
</style>
